...
class FoodNutritionAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'cuisine', 'meal_type', 'calories', 'health_score', 
        'popularity_score', 'is_active', 'created_at'
    ]
    list_filter = [
        'cuisine', 'meal_type', 'difficulty', 'is_active', 
        'is_vegetarian', 'is_vegan', 'is_high_protein', 
        'is_low_carb', 'is_gluten_free'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('name', 'english_name', 'description', 'cuisine', 'meal_type')
        }),
        ('制作信息', {
            'fields': ('difficulty', 'cooking_time', 'serving_size')
        }),
        ('主要营养成分', {
            'fields': ('calories', 'protein', 'fat', 'carbohydrates', 'dietary_fiber', 'sugar'),
            'classes': ['collapse']
        }),
        ('详细营养成分', {
            'fields': (
        ...
        ('食材和标签', {
            'fields': ('ingredients', 'allergens', 'tags')
        }),
        ('健康信息', {
            'fields': (
                'is_vegetarian', 'is_vegan', 'is_gluten_free', 'is_dairy_free',
                'is_low_carb', 'is_high_protein', 'is_low_fat', 'is_organic'
            )
        }),
        ('评分和媒体', {
            'fields': ('health_score', 'popularity_score', 'image_url', 'recipe_url')
        }),
        ('系统信息', {
        ...
        ('营养摘要', {
            'fields': ('macronutrients_display', 'nutrition_summary_display'),
            'classes': ['collapse']
        })
    )
    ...
    def update_popularity(self, request, queryset):
        ...
        for food in queryset:
            ...
            rating_avg = food.foodrandomizationlog_set.filter(
...
class FoodNutritionHistoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('food', 'user', 'serving_amount', 'rating')
        }),
        ('时间和备注', {
            'fields': ('consumed_at', 'notes')
        }),
        ('营养计算', {
            'fields': ('total_nutrition_display',),
            'classes': ['collapse']
        })
    )
...
class FoodRandomizationLogAdmin(admin.ModelAdmin):
    list_display = [
        'food', 'user', 'cuisine_filter', 'meal_type_filter', 
        'selected', 'rating', 'created_at'
    ]
    list_filter = [
        'selected', 'rating', 'cuisine_filter', 'meal_type_filter', 'created_at'
    ]
    ...
    fieldsets = (
        ('推荐信息', {
            'fields': ('food', 'user', 'session_id')
        }),
        ('筛选条件', {
            'fields': ('cuisine_filter', 'meal_type_filter')
        }),
        ('用户反馈', {
            'fields': ('selected', 'rating')
        }),
        ('时间信息', {
            'fields': ('created_at',)
        })
