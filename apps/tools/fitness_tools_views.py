    ...
    try:
        ...
        target_hr_ranges = {
            'light': {
                'min': int(max_hr * 0.5),
                'max': int(max_hr * 0.6),
                'description': '轻度活动，适合热身和恢复'
            },
            'moderate': {
                'min': int(max_hr * 0.6),
                'max': int(max_hr * 0.7),
                'description': '中等强度，适合有氧训练'
            },
            'vigorous': {
                'min': int(max_hr * 0.7),
                'max': int(max_hr * 0.85),
                'description': '高强度，适合心肺训练'
            },
            'maximum': {
                'min': int(max_hr * 0.85),
                'max': int(max_hr),
                'description': '最大强度，适合间歇训练'
            }
        }
        ...
        karvonen_ranges = {
            'light': {
                'min': int(resting_hr + hr_reserve * 0.5),
                'max': int(resting_hr + hr_reserve * 0.6)
            },
            'moderate': {
                'min': int(resting_hr + hr_reserve * 0.6),
                'max': int(resting_hr + hr_reserve * 0.7)
            },
            'vigorous': {
                'min': int(resting_hr + hr_reserve * 0.7),
                'max': int(resting_hr + hr_reserve * 0.85)
            },
            'maximum': {
                'min': int(resting_hr + hr_reserve * 0.85),
                'max': int(resting_hr + hr_reserve * 0.95)
            }
        }
