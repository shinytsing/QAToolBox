...
class GuitarTrainingSystem:
    ...
    ACHIEVEMENTS = {
        ...
        'level_up': {
            'name': '等级提升',
            'description': '提升到新的难度等级',
            'icon': '⭐',
            'points': 100
        },
        'master_chord': {
            'name': '和弦大师',
            'description': '掌握所有基础和弦',
            'icon': '🎯',
            'points': 150
        },
        'fingerpicking_pro': {
            'name': '指弹专家',
            'description': '掌握高级指弹技巧',
            'icon': '🎵',
            'points': 200
        },
        'song_master': {
            'name': '歌曲大师',
            'description': '学会10首完整歌曲',
            'icon': '🎼',
            'points': 300
        }
    }
    ...
    try:
        ...
        for i in range(0, chroma.shape[1], frame_length):
            ...
            chords_detected.append({
                'chord': best_chord,
                'time': i * librosa.get_duration(y=y, sr=sr) / chroma.shape[1],
                'duration': 4
            })
...
def get_achievements_api(request):
    ...
    try:
        ...
        all_achievement_types = [
            'traveler', 'explorer', 'prophet', 'collector', 
            'emotion-master', 'social-butterfly'
        ]
    ...
    try:
        ...
        if capsules.count() >= 7:
            achievement, created = Achievement.objects.get_or_create(
                ...
                defaults={'progress': capsules.count()}
            )
            if created:
