...
class IPDefenseSystem:
    ...
    def _system_unblock_ip(self, ip: str):
        ...
        try:
            ...
            try:
                subprocess.run(['ufw', 'allow', 'from', ip], 
                             check=True, capture_output=True)
...
def block_ip_api(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': success,
            'message': 'IP封禁成功' if success else 'IP封禁失败'
        })
...
def unblock_ip_api(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': success,
            'message': 'IP解封成功' if success else 'IP解封失败'
        })
