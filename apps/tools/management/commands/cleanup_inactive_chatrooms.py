...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'🧹 开始清理 {hours} 小时前无活动的聊天室...')
        )
        ...
        if not inactive_rooms:
            self.stdout.write(
                self.style.SUCCESS('✅ 没有需要清理的聊天室')
            )
            return
        
        self.stdout.write(
            self.style.WARNING(f'🗑️  找到 {len(inactive_rooms)} 个需要清理的聊天室:')
        )
        ...
        for room in inactive_rooms:
            ...
            self.stdout.write(
                f'  - 房间ID: {room_id_str[:8]}... | 最后活动: {last_activity} | 消息数: {message_count}'
            )
        
        if dry_run:
            self.stdout.write(
                self.style.WARNING('🔍 预览模式，不实际删除')
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f'✅ 成功清理 {deleted_count} 个聊天室')
        )
    ...
    def cleanup_rooms(self, rooms):
        ...
        for room in rooms:
            try:
                ...
                self.stdout.write(
                    f'  ✅ 删除房间 {room_id_str[:8]}... (包含 {message_count} 条消息)'
                )
                
            except Exception as e:
                room_id_str = str(room.room_id) if room.room_id else str(room.id)
                self.stdout.write(
                    self.style.ERROR(f'  ❌ 删除房间 {room_id_str[:8]}... 失败: {str(e)}')
                )
