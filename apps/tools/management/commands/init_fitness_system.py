...
from apps.tools.models.fitness_achievement_models import (
    FitnessAchievementModule, EnhancedFitnessAchievement
)
from apps.tools.models.exercise_library_models import (
    MuscleGroup, BodyPart, Equipment, Exercise
)
...
class Command(BaseCommand):
    ...
    def create_achievement_modules(self):
        ...
        for module_data in modules_data:
            module, created = FitnessAchievementModule.objects.get_or_create(
    ...
    def create_muscle_groups(self):
        ...
        for name, chinese_name, color in muscle_groups_data:
            muscle_group, created = MuscleGroup.objects.get_or_create(
                name=name,
                defaults={'chinese_name': chinese_name, 'color': color}
            )
            if created:
    ...
    def create_plan_categories(self):
        ...
        categories_data = [
            ...
            {
                'name': '减脂训练',
                'description': '燃脂塑形的综合训练计划',
                'icon': 'fas fa-fire',
                'color': '#e67e22'
            },
            {
                'name': '功能性训练',
                'description': '提升日常功能性的训练计划',
                'icon': 'fas fa-running',
                'color': '#27ae60'
            },
            {
                'name': '康复训练',
                'description': '伤后恢复和预防的训练计划',
                'icon': 'fas fa-heart',
                'color': '#9b59b6'
            }
        ]
        ...
        for category_data in categories_data:
            category, created = TrainingPlanCategory.objects.get_or_create(
