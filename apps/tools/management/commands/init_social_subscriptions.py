    ...
    def handle(self, *args, **options):
        ...
        if username:
            ...
            if not User.objects.exists():
                self.stdout.write(
                    self.style.ERROR('没有找到任何用户，请先创建用户')
                )
                return
        ...
        self.stdout.write(
            self.style.SUCCESS(f'成功初始化社交媒体订阅数据！')
        )
        ...
        platforms = [
            {
                ...
                'api_endpoint': 'https://api.xiaohongshu.com/v1',
                'rate_limit': 100
            },
            {
                'platform': 'douyin',
                'api_endpoint': 'https://api.douyin.com/v1',
                'rate_limit': 150
            },
            {
                'platform': 'netease',
            ...
            {
                'platform': 'weibo',
                'api_endpoint': 'https://api.weibo.com/v2',
                'rate_limit': 120
            },
            {
                'platform': 'bilibili',
                'api_endpoint': 'https://api.bilibili.com/x',
                'rate_limit': 180
            },
            {
                'platform': 'zhihu',
                'api_endpoint': 'https://api.zhihu.com/v4',
                'rate_limit': 80
            }
        ]
    ...
    def create_sample_notifications(self, subscriptions):
        ...
        notification_templates = {
            'newPosts': [
                {
                    'title': '发布了新内容',
                    'content': '刚刚发布了一篇精彩的内容，快去看看吧！'
                },
                {
                    'title': '更新了动态',
                    'content': '有新的动态更新，内容很有趣哦～'
                },
                {
                    'title': '分享了新作品',
                    'content': '新作品上线了，质量很高，值得关注！'
                }
            ],
            'newFollowers': [
                {
                    'title': '新增关注者',
                    'content': '获得了新的关注者，粉丝数量又增加了！'
                },
                {
                    'title': '粉丝增长',
                    'content': '粉丝数量突破新纪录，影响力在扩大！'
                }
            ],
            'profileChanges': [
                {
                    'title': '更新了个人资料',
                    'content': '个人资料有新的变化，可能更新了头像或简介。'
                },
                {
                    'title': '修改了签名',
                    'content': '个人签名有更新，看看有什么新想法！'
                }
            ]
        }
