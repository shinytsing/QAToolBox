...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'action',
            choices=['status', 'refresh', 'test', 'clear', 'add'],
            help='执行的操作'
        )
        parser.add_argument(
            '--proxy',
            type=str,
            help='添加代理时指定代理地址 (格式: ip:port)'
        )
        parser.add_argument(
            '--count',
            type=int,
    ...
    def show_status(self):
        ...
        available_proxies = [
            ...
            if p.proxy not in proxy_pool.failed_proxies and p.fail_count < 3
        ]
        ...
        if available_proxies:
            ...
            for i, proxy in enumerate(available_proxies[:5], 1):
                ...
                self.stdout.write(
                    f"{i}. {proxy.proxy} "
                    f"(响应: {response_time:.2f}s, 成功: {success_count}, 失败: {fail_count})"
                )

    def refresh_proxies(self):
        """刷新代理池"""
        self.stdout.write(
            self.style.WARNING('🔄 正在刷新代理池...')
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'✅ 代理池刷新完成! 从 {old_count} 个更新到 {new_count} 个代理'
            )
        )
    ...
    def test_proxies(self, count):
        """测试代理"""
        self.stdout.write(
            self.style.WARNING(f'🧪 正在测试前 {count} 个代理...')
        )
        ...
        for i, proxy_info in enumerate(test_proxies, 1):
            ...
            if proxy_pool._test_proxy(proxy_info):
                working_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f"  ✅ 可用 ({proxy_info.response_time or 0:.2f}s)")
                )
            else:
                self.stdout.write(
                    self.style.ERROR(f"  ❌ 失败")
                )
    ...
    def clear_failed_proxies(self):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'🧹 已清理 {failed_count} 个失败代理记录')
        )
    ...
    def add_proxy(self, proxy_address):
        ...
        if not proxy_address:
            self.stdout.write(
                self.style.ERROR('❌ 请使用 --proxy 参数指定代理地址')
            )
            return
        
        if ':' not in proxy_address:
            self.stdout.write(
                self.style.ERROR('❌ 代理格式错误，应为 ip:port')
            )
            return
        ...
        if existing:
            self.stdout.write(
                self.style.WARNING(f'⚠️ 代理 {proxy_address} 已存在')
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f'✅ 已添加代理: {proxy_address}')
        )
        ...
        if proxy_pool._test_proxy(new_proxy):
            self.stdout.write(
                self.style.SUCCESS(f'✅ 新代理测试成功!')
            )
        else:
            self.stdout.write(
                self.style.WARNING(f'⚠️ 新代理测试失败，但已添加到池中')
            )
