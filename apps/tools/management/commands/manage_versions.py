...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--features',
            type=str,
            help='功能列表，用逗号分隔 (例如: "功能1,功能2,功能3")'
        )
    ...
    def list_versions(self, tracker):
        ...
        for version in versions:
            ...
            self.stdout.write(
                f"v{version['version']} ({formatted_date})"
            )
    ...
    def add_version(self, tracker, options):
        ...
        if not all([version, title, features_str, description]):
            raise CommandError(
                '添加版本需要提供所有参数：--ver, --title, --features, --description'
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f'✅ 成功添加版本 v{version}: {title}')
        )
    ...
    def update_version(self, tracker, options):
        ...
        if success:
            self.stdout.write(
                self.style.SUCCESS(f'✅ 成功更新版本 v{version}')
            )
        else:
