...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if subscription_id:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'完成订阅 {subscription.target_user_name} 的检查')
                )
            except SocialMediaSubscription.DoesNotExist:
                self.stdout.write(
                    self.style.ERROR(f'订阅ID {subscription_id} 不存在')
                )
            return

        if continuous:
            self.stdout.write(
                self.style.SUCCESS('开始持续运行爬虫任务，根据订阅频率自动调度')
            )
            try:
                run_continuous_crawler()
            except KeyboardInterrupt:
                self.stdout.write(
                    self.style.WARNING('爬虫任务已停止')
                )
        else:
            # 单次运行
    ...
    def run_crawler_cycle(self):
        ...
        for subscription in active_subscriptions:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f'检查订阅 {subscription.target_user_name} 时出错: {str(e)}')
                )
                # 标记订阅为错误状态
    ...
    def generate_notification(self, subscription, notification_type):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'✓ 生成通知: {notification.title}')
        )
    ...
    def get_notification_template(self, subscription, notification_type):
        ...
        templates = {
            'newPosts': [
                ...
                {
                    'title': f'{subscription.target_user_name} 更新了动态',
                    'content': f'有新的动态更新，内容很有趣哦～'
                },
                {
                    'title': f'{subscription.target_user_name} 分享了新作品',
                    'content': f'新作品上线了，质量很高，值得关注！'
                }
            ],
            'newFollowers': [
                {
                    'title': f'{subscription.target_user_name} 新增关注者',
                    'content': f'获得了新的关注者，粉丝数量又增加了！'
                },
                {
                    'title': f'{subscription.target_user_name} 粉丝增长',
                    'content': f'粉丝数量突破新纪录，影响力在扩大！'
                }
            ],
            'profileChanges': [
                {
                ...
                {
                    'title': f'{subscription.target_user_name} 修改了签名',
                    'content': f'个人签名有更新，看看有什么新想法！'
                }
            ]
        }

        return random.choice(templates.get(notification_type, [{
