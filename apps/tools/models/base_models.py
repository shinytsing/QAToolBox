    ...
    def get_user_tool_usage(cls, user, tool_type=None, days=30):
        ...
        if result is None:
            ...
            result = {
                ...
                'tool_breakdown': list(queryset.values('tool_type').annotate(
    ...
    def get_tool_statistics(cls, tool_type=None, days=30):
        ...
        if result is None:
            ...
            result = {
                ...
                'daily_usage': list(queryset.extra(
    ...
    def get_user_activity_summary(cls, user, days=30):
        ...
        if result is None:
            ...
            result = {
                ...
                'activity_breakdown': list(queryset.values('activity_type').annotate(
                    count=models.Count('id')
                ).order_by('-count')),
                'recent_activities': list(queryset.order_by('-timestamp')[:10].values(
                    'activity_type', 'description', 'timestamp'
                )),
                'daily_activity': list(queryset.extra(
...
class FeatureFlag(models.Model):
    ...
    def is_enabled_for_user(self, user):
        ...
        if self.flag_type == 'BOOLEAN':
            ...
            now = timezone.now()
            return (not self.start_time or now >= self.start_time) and \
