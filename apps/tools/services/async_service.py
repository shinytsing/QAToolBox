...
from apps.tools.services.cache_service import (
    UserCacheService, ChatCacheService, TimeCapsuleCacheService, StatsCacheService
)
    ...
    try:
        ...
        stats = {
            ...
            'active_rooms': ChatRoom.objects.filter(
                user1=user, status='active'
            ).count() + ChatRoom.objects.filter(
                user2=user, status='active'
            ).count(),
...
def process_time_capsule_unlock():
    ...
    try:
        ...
        # 查找需要解锁的时光胶囊
        unlockable_capsules = TimeCapsule.objects.filter(
            unlock_time__lte=now,
        ...
        for capsule in unlockable_capsules:
            try:
                ...
                send_notification_email.delay(
                    capsule.user.id,
                    "时光胶囊解锁通知",
                    f"您的时光胶囊 '{capsule.title or '未命名'}' 已经解锁！"
                )
                count += 1
...
def cache_user_data(user_id: int):
    ...
    try:
        ...
        # 缓存用户时光胶囊
        capsules = list(TimeCapsule.objects.filter(user=user).values(
            'id', 'title', 'content', 'emotions', 'visibility', 'created_at'
        ))
        TimeCapsuleCacheService.cache_user_capsules(user_id, capsules)
        
        # 缓存活跃聊天室
        active_rooms = list(ChatRoom.objects.filter(
            user1=user, status='active'
        ).values('id', 'room_id', 'created_at'))
        active_rooms.extend(list(ChatRoom.objects.filter(
            user2=user, status='active'
        ).values('id', 'room_id', 'created_at')))
...
def warm_up_cache():
    ...
    try:
        ...
        # 预热公开时光胶囊
        public_capsules = list(TimeCapsule.objects.filter(
            visibility='public'
        ).values('id', 'title', 'content', 'emotions', 'created_at')[:100])
