...
class DatabaseCleanupService:
    ...
    def _cleanup_tool_usage_logs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取要删除的记录数量
            total_count = ToolUsageLog.objects.filter(
            ...
            deleted_count = 0
            while True:
                batch = ToolUsageLog.objects.filter(
                    created_at__lt=cutoff_date
                )[:batch_size]
    ...
    def _cleanup_social_media_subscriptions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取要删除的记录数量
            total_count = SocialMediaSubscription.objects.filter(
                created_at__lt=cutoff_date,
                status='inactive'
            ).count()
            ...
            deleted_count = 0
            while True:
                batch = SocialMediaSubscription.objects.filter(
                    created_at__lt=cutoff_date,
                    status='inactive'
                )[:batch_size]
    ...
    def get_cleanup_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats['social_media_subscriptions'] = {
                ...
                'old_count': SocialMediaSubscription.objects.filter(
                    created_at__lt=cutoff_date,
                    status='inactive'
                ).count(),
                'retention_days': retention_days
