...
class EnhancedTravelService:
    ...
    def get_real_travel_guide(self, destination: str, travel_style: str, 
        ...
        try:
            ...
            enhanced_content = self._generate_enhanced_content_with_deepseek(
                ...
                interests, attractions_data, food_data, weather_info
            )
            
            # 8. 合成最终攻略
            final_guide = self._synthesize_final_guide(
                destination, travel_style, budget_range, travel_duration,
                interests, geo_info, weather_info, attractions_data, 
                food_data, transport_data, accommodation_data, enhanced_content
            )
    ...
    def _generate_enhanced_content_with_deepseek(self, destination: str, travel_style: str,
                                               budget_range: str, travel_duration: str,
                                               interests: List[str], attractions_data: Dict,
                                               food_data: Dict, weather_info: Dict) -> Dict:
        """使用DeepSeek生成增强内容"""
        ...
        try:
            # 构建包含真实数据的提示词
            attractions_text = "\n".join([f"- {att['name']}: {att['description'][:100]}..." 
                                        for att in attractions_data.get('attractions', [])[:5]])
            foods_text = "\n".join([f"- {food['name']}: {food['description'][:100]}..." 
                                  for food in food_data.get('foods', [])[:5]])
...
请确保所有建议都基于提供的真实数据，避免虚假信息。"""
    ...
    def _synthesize_final_guide(self, destination: str, travel_style: str,
                               ...
                               weather_info: Dict, attractions_data: Dict,
                               food_data: Dict, transport_data: Dict,
                               accommodation_data: Dict, enhanced_content: Dict) -> Dict:
        """合成最终攻略"""
    ...
    def _generate_realistic_budget(self, days: int, budget_range: str, 
                                 attractions_data: Dict, accommodation_data: Dict) -> Dict:
        """生成真实预算估算"""
        base_costs = {
    ...
    def _generate_realistic_tips(self, destination: str, weather_info: Dict, 
                                attractions_data: Dict) -> List[str]:
        """生成真实旅行贴士"""
        tips = []
    ...
    def _generate_fallback_with_deepseek(self, destination: str, travel_style: str,
                                       budget_range: str, travel_duration: str,
                                       interests: List[str]) -> Dict:
        """使用DeepSeek生成备用攻略"""
        if not self.deepseek_client:
...
请确保信息真实可靠，避免虚假信息。"""
    ...
    def _generate_basic_fallback(self, destination: str, travel_style: str,
                               budget_range: str, travel_duration: str,
                               interests: List[str]) -> Dict:
        """生成基础备用攻略"""
        return {
