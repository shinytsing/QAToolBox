...
from ..models import (
    Feature, FeatureRecommendation, UserFeaturePermission, 
    UserFirstVisit
)
...
class FeatureRecommendationEngine:
    ...
    def get_recommendations_for_user(self, user, limit=None, context=None):
        ...
        if remaining_limit > 0:
            ...
            remaining_features = [f for f in available_features 
                                if f not in [r['feature'] for r in recommendations]]
    ...
    def _get_available_features(self, user):
        ...
        # 检查用户特定权限
        user_permissions = UserFeaturePermission.objects.filter(
            user=user, is_visible=True, is_allowed=True
    ...
    def _get_personalized_recommendations(self, user, features, limit):
        ...
        for feature, score in scored_features[:limit]:
            ...
            recommendations.append({
                ...
                'metadata': {
                    'user_mode': user_mode,
                    'category_match': feature.category in preferred_categories
                }
            })
    ...
    def _calculate_personalized_score(self, user, feature, user_mode, preferred_categories):
        ...
        user_history = FeatureRecommendation.objects.filter(
            user=user, 
            feature__category=feature.category,
            action='clicked'
        ).count()
    ...
    def _get_popular_recommendations(self, features, limit):
        ...
        features_sorted = sorted(
            filtered_features, 
            key=lambda f: (f.popularity_score, f.total_usage_count, f.recommendation_weight),
            reverse=True
        )
        ...
        for feature in features_sorted[:limit]:
            ...
            recommendations.append({
                ...
                'metadata': {
                    ...
                    'usage_count': feature.total_usage_count
                }
            })
    ...
    def record_user_action(self, user, feature, action, session_id=None, context=None):
        ...
        if action == 'clicked':
            ...
            try:
                ...
                UserFirstVisit.objects.create(
                    user=user,
                    total_feature_usage=1
                )
    ...
    def get_recommendation_stats(self, user=None, days=30):
        ...
        if stats['shown_recommendations'] > 0:
            stats['click_rate'] = (stats['clicked_recommendations'] / 
                                 stats['shown_recommendations']) * 100
        else:
            stats['click_rate'] = 0
        
        # 热门功能统计
        popular_features = Feature.objects.filter(
            featurerecommendation__created_at__gte=since,
            featurerecommendation__action='clicked'
        )
        
        if user:
            popular_features = popular_features.filter(
                featurerecommendation__user=user
            )
        
        popular_features = popular_features.annotate(
            click_count=Count('featurerecommendation')
        ...
        # 算法效果统计
        algorithm_stats = base_query.filter(action='clicked').values(
