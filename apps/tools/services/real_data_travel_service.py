...
class RealDataTravelService:
    ...
    def get_real_travel_guide(self, destination: str, travel_style: str, 
        ...
        try:
            ...
            attractions = self._get_data_with_fallback(
                'attractions', self._get_real_attractions_with_deepseek, 
                destination, travel_style, interests
            )
            
            # 获取美食数据
            logger.info("🔍 获取foods数据...")
            foods = self._get_data_with_fallback(
                'foods', self._get_real_foods_with_deepseek,
            ...
            accommodations = self._get_data_with_fallback(
                'accommodations', self._get_real_accommodations_with_deepseek,
                destination, budget_range
            )
            
            # 获取交通数据
            logger.info("🔍 获取transport数据...")
            transport = self._get_data_with_fallback(
                'transport', self._get_real_transport_with_deepseek,
                destination
            )
            ...
            complete_guide = self._generate_complete_guide_with_deepseek(
                ...
                interests, attractions, foods, accommodations, transport, weather_info, geo_info
            )
            ...
            return self._generate_fallback_with_deepseek(
...
请确保返回的是有效的JSON格式，不要包含任何解释文字。"""
    ...
    def _generate_complete_guide_with_deepseek(self, destination: str, travel_style: str,
                                             budget_range: str, travel_duration: str,
                                             interests: List[str], attractions: List[Dict],
                                             foods: List[Dict], accommodations: List[Dict],
                                             transport: Dict, weather_info: Dict,
                                             geo_info: Dict) -> str:
...
请确保所有信息都基于提供的真实数据，内容要极其详细实用，便于游客参考。每个部分都要有具体的建议和实用的信息。"""
    ...
    def _synthesize_final_guide(self, destination: str, travel_style: str,
                               budget_range: str, travel_duration: str,
                               interests: List[str], geo_info: Dict,
                               weather_info: Dict, attractions: List[Dict],
                               foods: List[Dict], transport: Dict,
                               accommodations: List[Dict], complete_guide: str) -> Dict:
        """合成最终攻略 - 新格式"""
        try:
            ...
            detailed_guide_text = self._generate_detailed_guide_text(
                overview, itinerary, attractions_section, foods_section, 
                transport_section, budget_section, tips_section, accommodations
            )
    ...
    def _generate_realistic_budget(self, days: int, budget_range: str, 
                                 attractions: List[Dict], accommodations: List[Dict]) -> Dict:
        """生成真实预算估算"""
        base_costs = {
    ...
    def _generate_realistic_daily_schedule(self, destination: str, days: int,
                                         attractions: List[str], foods: List[str],
                                         travel_style: str, weather_info: Dict) -> List[Dict]:
        """生成真实每日行程"""
        schedules = []
    ...
    def _generate_realistic_tips(self, destination: str, weather_info: Dict, 
                                attractions: List[Dict]) -> List[str]:
        """生成真实旅行贴士"""
        tips = []
    ...
    def _generate_fallback_guide(self, destination: str, travel_style: str,
                               budget_range: str, travel_duration: str,
                               interests: List[str]) -> Dict:
        """生成备用攻略"""
        return {
    ...
    def _get_fallback_attractions(self, destination: str, travel_style: str, interests: List[str]) -> List[Dict]:
        ...
        common_attractions = {
            '武汉': [
                {'name': '黄鹤楼', 'ticket_price': '80元', 'opening_hours': '8:00-17:00', 'reason': '武汉地标建筑', 'transport': '地铁4号线', 'duration': '2小时', 'type': '历史建筑'},
                {'name': '东湖', 'ticket_price': '免费', 'opening_hours': '全天开放', 'reason': '中国最大的城中湖', 'transport': '公交或地铁', 'duration': '半天', 'type': '自然景观'},
                {'name': '户部巷', 'ticket_price': '免费', 'opening_hours': '全天开放', 'reason': '武汉特色小吃街', 'transport': '地铁2号线', 'duration': '2小时', 'type': '美食街'},
            ],
            '北京': [
                {'name': '故宫', 'ticket_price': '60元', 'opening_hours': '8:30-17:00', 'reason': '明清皇宫', 'transport': '地铁1号线', 'duration': '4小时', 'type': '历史建筑'},
                {'name': '天安门广场', 'ticket_price': '免费', 'opening_hours': '全天开放', 'reason': '中国政治中心', 'transport': '地铁1号线', 'duration': '1小时', 'type': '广场'},
                {'name': '长城', 'ticket_price': '40元', 'opening_hours': '8:00-17:00', 'reason': '世界文化遗产', 'transport': '旅游大巴', 'duration': '半天', 'type': '历史建筑'},
            ],
            '上海': [
                {'name': '外滩', 'ticket_price': '免费', 'opening_hours': '全天开放', 'reason': '黄浦江畔景观', 'transport': '地铁2号线', 'duration': '2小时', 'type': '城市景观'},
                {'name': '东方明珠', 'ticket_price': '220元', 'opening_hours': '8:30-21:30', 'reason': '上海地标', 'transport': '地铁2号线', 'duration': '2小时', 'type': '现代建筑'},
                {'name': '豫园', 'ticket_price': '45元', 'opening_hours': '8:45-16:45', 'reason': '明代园林', 'transport': '地铁10号线', 'duration': '2小时', 'type': '古典园林'},
            ]
        }
        
        return common_attractions.get(destination, [
            {'name': f'{destination}著名景点', 'ticket_price': '50元', 'opening_hours': '9:00-17:00', 'reason': '当地特色', 'transport': '公交', 'duration': '2小时', 'type': '景点'}
        ])
    ...
    def _get_fallback_foods(self, destination: str, interests: List[str]) -> List[Dict]:
        ...
        return common_foods.get(destination, [
            {'name': f'{destination}特色美食', 'price': '30-50元', 'location': '当地餐厅', 'description': '当地特色', 'type': '特色菜'}
        ])
    ...
    def _generate_fallback_complete_guide(self, destination: str, travel_style: str,
                                        budget_range: str, travel_duration: str,
...
"""
    ...
    def _generate_detailed_guide_text(self, overview: Dict, itinerary: Dict, 
                                    attractions_section: Dict, foods_section: Dict,
                                    transport_section: Dict, budget_section: Dict, 
                                    tips_section: Dict, accommodations: List[Dict]) -> str:
        """生成详细攻略文本（用于PDF导出）"""
        try:
            content = []
