...
class TravelDataService:
    ...
    def get_travel_guide_data(self, destination: str, travel_style: str, 
                             budget_min: int, budget_max: int, budget_amount: int, budget_range: str, travel_duration: str, 
        ...
        try:
            ...
            # 3. 智能合成阶段
            print("🤖 阶段3: 智能合成...")
            final_guide = self._智能合成阶段(destination, travel_style, 
                                            budget_min, budget_max, budget_amount, budget_range, travel_duration, 
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    'weather': current_condition['lang_zh'][0]['value'] if 'lang_zh' in current_condition else current_condition['weatherDesc'][0]['value'],
    ...
    def _信息结构化(self, raw_data: Dict, destination: str = "目的地") -> Dict:
        ...
        try:
            # 从搜索数据提取
            if ('search_data' in raw_data and 
                isinstance(raw_data['search_data'], dict) and 
                'error' not in raw_data['search_data']):
            ...
            # 从维基百科数据提取
            if ('wiki_data' in raw_data and 
                isinstance(raw_data['wiki_data'], dict) and 
                'error' not in raw_data['wiki_data']):
                wiki_data = raw_data['wiki_data']
                structured_data['简介'] = wiki_data.get('extract', f'{destination}是一个美丽的旅游目的地')
            
            # 从天气数据提取
            if ('weather' in raw_data and 
                isinstance(raw_data['weather'], dict) and 
                'error' not in raw_data['weather']):
                structured_data['天气'] = raw_data['weather']
            
            # 从OpenTripMap数据提取景点详情
            if ('opentripmap' in raw_data and 
                isinstance(raw_data['opentripmap'], dict) and 
                'error' not in raw_data['opentripmap']):
            ...
            # 从国家信息数据提取
            if ('country' in raw_data and 
                isinstance(raw_data['country'], dict) and 
                'error' not in raw_data['country']):
                structured_data['国家信息'] = raw_data['country']
            
            # 从地理位置数据提取
            if ('geolocation' in raw_data and 
                isinstance(raw_data['geolocation'], dict) and 
                'error' not in raw_data['geolocation']):
                structured_data['地理位置'] = raw_data['geolocation']
            
            # 从汇率数据提取
            if ('currency' in raw_data and 
                isinstance(raw_data['currency'], dict) and 
                'error' not in raw_data['currency']):
                structured_data['汇率信息'] = raw_data['currency']
            
            # 从时区数据提取
            if ('timezone' in raw_data and 
                isinstance(raw_data['timezone'], dict) and 
                'error' not in raw_data['timezone']):
    ...
    def _智能合成阶段(self, destination: str, travel_style: str, 
                      budget_min: int, budget_max: int, budget_amount: int, budget_range: str, travel_duration: str, 
                      interests: List[str], structured_data: Dict) -> Dict:
        """智能合成阶段 - 使用本地算法生成攻略"""
        try:
            # 生成智能攻略
            guide_data = self._generate_smart_guide(destination, travel_style, 
                                                   budget_min, budget_max, budget_amount, budget_range, travel_duration, 
                                                   interests, structured_data)
    ...
    def _generate_smart_guide(self, destination: str, travel_style: str, 
                             budget_min: int, budget_max: int, budget_amount: int, budget_range: str, travel_duration: str, 
                             interests: List[str], structured_data: Dict) -> Dict:
        """生成智能攻略"""
        ...
        # 生成详细攻略
        detailed_guide = self._generate_detailed_guide_text(destination, adjusted_attractions, 
                                                           adjusted_foods, structured_data['贴士'], 
                                                           travel_style, budget_min, budget_max, budget_amount, budget_range, travel_duration)
        
        # 生成增强版攻略内容
        enhanced_guide = self._generate_enhanced_guide_content(destination, structured_data, travel_style, budget_min, budget_max, budget_amount, budget_range)
        ...
        return {
            ...
            'daily_schedule': self._generate_daily_schedule(destination, days, {
                '景点': adjusted_attractions,
                '美食': adjusted_foods,
                '贴士': structured_data['贴士']
            }),
            'activity_timeline': self._generate_activity_timeline(days),
            'cost_breakdown': self._generate_cost_breakdown(destination, days, budget_min, budget_max, budget_amount, budget_range, {
                '景点': adjusted_attractions,
                '美食': adjusted_foods,
                '贴士': structured_data['贴士']
            }),
    ...
    def _generate_budget_estimate(self, budget_min: int, budget_max: int, budget_amount: int, budget_range: str, days: int) -> Dict:
        """生成预算估算 - 基于预算范围智能分配"""
        # 按照经验比例分配预算
        allocation_ratios = {
            '住宿': 0.4,     # 40%
    ...
    def _generate_enhanced_guide_content(self, destination: str, structured_data: Dict, 
                                       travel_style: str, budget_min: int, budget_max: int, budget_amount: int, budget_range: str) -> Dict:
        """生成增强版攻略内容"""
        currency_info = structured_data.get('汇率信息', {})
    ...
    def _generate_attraction_info(self, attraction_details: List[Dict]) -> Dict:
        ...
        for attraction in attraction_details:
            ...
            if 'cultural' in attraction_type or 'museum' in attraction_type:
                categorized_attractions['cultural'].append({
                    'name': name,
                    'description': description,
                    'type': '文化景点'
                })
            elif 'historic' in attraction_type or 'castle' in attraction_type:
                categorized_attractions['historic'].append({
                    'name': name,
                    'description': description,
                    'type': '历史景点'
                })
            elif 'natural' in attraction_type or 'park' in attraction_type:
                categorized_attractions['natural'].append({
                    'name': name,
                    'description': description,
                    'type': '自然景点'
                })
            else:
                categorized_attractions['entertainment'].append({
                    'name': name,
                    'description': description,
                    'type': '娱乐景点'
                })
        ...
        return {
            'attractions': [
                ...
                },
                {
                    ...
                    'description': f'{destination}的自然风光优美，是放松身心的好去处。'
                }
            ],
            'tips': [
                {
                    'title': f'{destination}旅游贴士',
                    'content': f'建议提前了解{destination}的天气情况，准备合适的衣物。'
                }
            ]
        }
    ...
    def _generate_cost_breakdown(self, destination: str, days: int, budget_min: int, budget_max: int, budget_amount: int, budget_range: str, real_data: Dict) -> Dict:
        """生成费用明细 - 基于预算范围智能分配"""
        # 分配比例
        allocation = {
    ...
    def _generate_detailed_guide_text(self, destination: str, attractions: List[str], 
                                    foods: List[str], tips: List[str], 
                                    travel_style: str, budget_min: int, budget_max: int, budget_amount: int, budget_range: str, 
                                    travel_duration: str) -> str:
        """生成详细攻略文本"""
        days = self._parse_travel_duration(travel_duration)
        ...
        # 生成详细的预算分析
        budget_info = self._generate_detailed_budget_info(destination, days, budget_min, budget_max, budget_amount, budget_range, travel_style)
...
"""
    ...
    def _generate_detailed_daily_schedules(self, destination: str, days: int, attractions: List[str], foods: List[str], travel_style: str) -> str:
        """生成详细的每日行程安排"""
        schedules = []
...
"""
    ...
    def _generate_detailed_budget_info(self, destination: str, days: int, budget_min: int, budget_max: int, budget_amount: int, budget_range: str, travel_style: str) -> str:
        """生成详细的预算分析 - 基于具体预算金额"""
        # 分配比例
        allocation = {
            '住宿': 0.4,
