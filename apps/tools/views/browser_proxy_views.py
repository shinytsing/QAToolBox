...
def configure_browser_proxy(request):
    ...
    try:
        ...
        if browser_type == 'all':
            ...
            return JsonResponse({
                'success': False,
                'message': f'不支持的浏览器类型: {browser_type}'
            })
        ...
        return JsonResponse({
            ...
            'message': f'成功配置 {success_count}/{total_count} 个代理',
            'results': results
        })
...
def disable_browser_proxy(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': success_count > 0,
            'message': f'成功禁用 {success_count} 个代理',
            'results': results
        })
    ...
    try:
        ...
        try:
            ...
            if result != 0:
                return JsonResponse({
                    'success': False,
                    'message': f'端口 {proxy_host}:{proxy_port} 未开放',
                    'error': 'Port not open'
                })
        except Exception as e:
            return JsonResponse({
                'success': False,
                'message': f'端口测试失败: {str(e)}',
                'error': str(e)
            })
