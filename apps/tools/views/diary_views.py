...
from ..models import LifeDiaryEntry, LifeStatistics
from .base import (
...
def diary_dashboard(request):
    ...
    # 获取心情分布
    mood_distribution = LifeDiaryEntry.objects.filter(
        user=user,
...
def create_diary_entry(request):
    ...
    try:
        ...
        existing_entry = LifeDiaryEntry.objects.filter(
            user=user, 
            date=today
        ).first()
...
def diary_calendar(request):
    ...
    # 获取指定月份的日记
    entries = LifeDiaryEntry.objects.filter(
        user=user,
        date__year=year,
        date__month=month
    ).values('date', 'mood', 'id')
...
def mood_analysis(request):
    ...
    # 获取心情分布
    mood_stats = LifeDiaryEntry.objects.filter(
        user=user,
        date__gte=timezone.now().date() - timezone.timedelta(days=days)
    ).values('mood').annotate(
        count=Count('id'),
        avg_words=Avg('content__length')
    ).order_by('-count')
    
    # 获取心情趋势
    mood_trend = LifeDiaryEntry.objects.filter(
        user=user,
        date__gte=timezone.now().date() - timezone.timedelta(days=days)
    ).values('date', 'mood').order_by('date')
...
def batch_delete_diary_entries(request):
    ...
    try:
        ...
        user = request.user
        deleted_count = LifeDiaryEntry.objects.filter(
            user=user,
            id__in=entry_ids
        ).delete()[0]
...
def export_diary_entries(request):
    ...
    try:
        ...
        entries = list(queryset.order_by('-date').values(
            'title', 'content', 'mood', 'date', 'tags', 'mood_note'
        ))
