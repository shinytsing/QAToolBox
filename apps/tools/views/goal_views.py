...
def life_goals_dashboard(request):
    ...
    # 获取最近进度更新
    recent_progress = LifeGoalProgress.objects.filter(
        goal__user=user
    ).select_related('goal').order_by('-created_at')[:10]
    
    # 获取目标完成率趋势
    completion_trend = LifeStatistics.objects.filter(
        user=user
    ).order_by('-date')[:30]
...
def goals_statistics(request):
    ...
    progress_trend = LifeGoalProgress.objects.filter(
        goal__user=user,
        date__gte=start_date
    ).values('date').annotate(
        avg_progress=Avg('progress_value'),
        total_updates=Count('id')
    ).order_by('date')
    
    # 获取类别分布
    category_distribution = LifeGoal.objects.filter(
        user=user
    ...
    # 获取优先级分布
    priority_distribution = LifeGoal.objects.filter(
        user=user
...
def batch_update_goals(request):
    ...
    try:
        ...
        user = request.user
        updated_count = LifeGoal.objects.filter(
            user=user,
            id__in=goal_ids
        ).update(**updates)
        ...
        }, message=f"成功更新 {updated_count} 个目标")
...
def batch_delete_goals(request):
    ...
    try:
        ...
        user = request.user
        deleted_count = LifeGoal.objects.filter(
            user=user,
            id__in=goal_ids
        ).delete()[0]
        ...
        }, message=f"成功删除 {deleted_count} 个目标")
...
def goal_reminders(request):
    ...
    # 长期未更新的目标
    long_inactive_goals = LifeGoal.objects.filter(
        user=user,
        status='active'
    ).annotate(
        last_progress_date=Max('lifegoalprogress__date')
    ).filter(
        Q(last_progress_date__isnull=True) | 
        Q(last_progress_date__lt=today - timedelta(days=30))
    ).order_by('-created_at')
