...
def get_dashboard_stats_api(request):
    ...
    try:
        ...
        active_relationships = PersonProfile.objects.filter(
            user=user,
            last_interaction_date__gte=recent_date
        ).count()
...
def get_relationship_tags_api(request):
    ...
    try:
        ...
        for tag in tags:
            # 计算使用该标签的用户人员数量
            count = PersonProfile.objects.filter(
                user=request.user,
    ...
    try:
        ...
        graph_data = {
            ...
            'statistics': {
                ...
                'most_connected': (most_connected_profile.nickname or most_connected_profile.name) if most_connected_profile else '无'
            }
        }
