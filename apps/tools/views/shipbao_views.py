...
def shipbao_favorites_api(request):
    ...
    try:
        ...
        # 检查是否已经收藏
        existing_favorite = ShipBaoFavorite.objects.filter(
            user=request.user,
            item=item
        ).first()
        ...
        if action == 'add':
            ...
            # 创建收藏记录
            ShipBaoFavorite.objects.create(
                user=request.user,
                item=item
            )
...
def shipbao_initiate_transaction_api(request):
    ...
    try:
        ...
        existing_transaction = ShipBaoTransaction.objects.filter(
            item=item,
            buyer=request.user,
            status__in=['initiated', 'negotiating']
        ).first()
...
def shipbao_contact_seller_api(request):
    ...
    try:
        ...
        # 首先检查是否已经有咨询记录
        existing_inquiry = ShipBaoInquiry.objects.filter(
            item=item,
            buyer=request.user
        ).first()
        ...
        if existing_inquiry:
            if existing_inquiry.status == 'pending' and not existing_inquiry.chat_room:
                ...
                queue_position = ShipBaoInquiry.objects.filter(
                    item=item,
                    status='pending',
                    created_at__lt=existing_inquiry.created_at
                ).count() + 1
        ...
        try:
            with transaction.atomic():
                ...
                queue_position = ShipBaoInquiry.objects.filter(
                    item=item,
                    status='pending',
                    created_at__lt=inquiry.created_at
                ).count() + 1
                
                # 如果队列较短或者是高优先级用户或聊天室过期，立即创建聊天室
                pending_count = ShipBaoInquiry.objects.filter(
                    item=item,
                    status='pending'
                ).count()
                
                should_create_immediately = (
                    reuse_inquiry is not None or  # 聊天室过期，直接创建新的
                    pending_count <= 3 or  # 队列较短
                    inquiry.priority_score > 100  # 高优先级用户
                )
                
                if should_create_immediately:
                    # 查找是否已有活跃的聊天室
                    chat_room = ChatRoom.objects.filter(
                        Q(user1=request.user, user2=item.seller) |
                        Q(user1=item.seller, user2=request.user)
                    ).filter(
                        room_type='private',
                        status='active'  # 只查找活跃的聊天室
                    ).first()
...
def shipbao_inquiry_queue_api(request, item_id):
    ...
    try:
        ...
        user_inquiry = None
        if request.user.is_authenticated:
            user_inquiry = ShipBaoInquiry.objects.filter(
                item=item,
                buyer=request.user
            ).first()
        
        # 获取队列统计
        total_pending = ShipBaoInquiry.objects.filter(
            item=item,
            status='pending'
        ).count()
        
        total_responded = ShipBaoInquiry.objects.filter(
            item=item,
            status='responded'
        ).count()
        ...
        if user_inquiry:
            ...
            if user_inquiry.status == 'pending':
                position = ShipBaoInquiry.objects.filter(
                    item=item,
                    status='pending',
                    created_at__lt=user_inquiry.created_at
                ).count() + 1
...
def shipbao_want_item_api(request):
    ...
    try:
        ...
        # 检查是否已经想要过
        existing_want = ShipBaoWantItem.objects.filter(
            user=request.user,
            item=item
        ).first()
        ...
        if action == 'add':
            ...
            # 创建想要记录
            ShipBaoWantItem.objects.create(
                user=request.user,
                item=item,
                message=message
            )
...
def shipbao_transaction_status_api(request, item_id):
    ...
    try:
        ...
        existing_transaction = ShipBaoTransaction.objects.filter(
            item=item,
            buyer=request.user,
            status__in=['initiated', 'negotiating', 'confirmed', 'completed']
        ).first()
