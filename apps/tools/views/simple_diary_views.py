...
def create_diary_entry_safe(user, date, title, content, mood, entry_type, **kwargs):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            ''', [
                date, title, content, mood, entry_type, user.id,
                timezone.now(), timezone.now(),
                '', '[]', '', '[]', '', '', '{}', '', '',
                len(content) if content else 0, 1, False, True
            ])
...
def simple_diary_home(request):
    ...
    if user:
        ...
        # 获取最近的成就
        recent_achievements = DiaryAchievement.objects.filter(
            user=user, 
            is_completed=True
        ...
        month_entries_count = LifeDiaryEntry.objects.filter(
            user=user,
...
def diary_calendar_view(request):
    ...
    entries = LifeDiaryEntry.objects.filter(
        ...
        'date', 'mood', 'entry_type', 'word_count', 'id'
    )
...
def diary_history_timeline(request):
    ...
    for entry in page_entries:
        ...
        last_year_entry = LifeDiaryEntry.objects.filter(
            user=user,
            date=last_year_date
        ).first()
