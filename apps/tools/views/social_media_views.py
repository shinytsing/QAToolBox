...
from ..models import SocialMediaSubscription, SocialMediaNotification, ToolUsageLog
from .base import (
...
def update_subscription_api(request):
    ...
    try:
        ...
        subscription = get_object_or_404(SocialMediaSubscription, 
...
def mark_notification_read_api(request):
    ...
    try:
        ...
        notification = get_object_or_404(SocialMediaNotification, 
...
def get_subscription_stats_api(request):
    ...
    try:
        ...
        # 获取平台分布
        platform_distribution = SocialMediaSubscription.objects.filter(
            user=user
...
def batch_update_subscriptions_api(request):
    ...
    try:
        ...
        user = request.user
        updated_count = SocialMediaSubscription.objects.filter(
            user=user,
        ...
        }, message=f"成功更新 {updated_count} 个订阅")
...
def batch_delete_subscriptions_api(request):
    ...
    try:
        ...
        user = request.user
        deleted_count = SocialMediaSubscription.objects.filter(
            user=user,
            id__in=subscription_ids
        ).delete()[0]
        ...
        }, message=f"成功删除 {deleted_count} 个订阅")
...
class SocialMediaAPIView(BaseView, CachedViewMixin, PaginationMixin, SearchMixin, FilterMixin, OrderMixin):
    ...
    def get(self, request):
        ...
        # 应用排序
        allowed_order_fields = ['created_at', '-created_at', 'updated_at', '-updated_at', 'last_check_time', '-last_check_time']
