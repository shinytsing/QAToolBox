...
# 导入相关模型
from ..models import (
    TravelPost, TravelCity, TravelPostLike, TravelPostFavorite, 
    TravelPostComment, UserGeneratedTravelGuide
)
...
def travel_post_list_api(request):
    ...
    try:
        ...
        # 构建查询
        queryset = TravelPost.objects.filter(
            is_public=True, 
            is_approved=True
        ).select_related('user').prefetch_related('cities')
        
        # 搜索筛选
        if search:
            queryset = queryset.filter(
                Q(title__icontains=search) |
                Q(cities__name__icontains=search) |
                Q(cities__country__icontains=search)
            ).distinct()
...
def travel_post_create_api(request):
    ...
    try:
        ...
        if cover_image.content_type not in allowed_types:
            return JsonResponse({
                'success': False,
                'error': '封面图格式不支持，请上传JPG、PNG或WebP格式'
            }, status=400)
...
def travel_post_like_api(request, post_id):
    ...
    try:
        ...
        like, created = TravelPostLike.objects.get_or_create(
            user=request.user,
            post=post
        )
...
def travel_post_favorite_api(request, post_id):
    ...
    try:
        ...
        favorite, created = TravelPostFavorite.objects.get_or_create(
            user=request.user,
            post=post
        )
...
def travel_post_comment_api(request, post_id):
    ...
    try:
        ...
        if request.method == 'GET':
            # 获取评论列表
            comments = TravelPostComment.objects.filter(
                post=post,
                parent=None  # 只获取顶级评论
            ).select_related('user').prefetch_related('travelpostcomment_set').order_by('-created_at')
            ...
            comment = TravelPostComment.objects.create(
                user=request.user,
                post=post,
                parent=parent,
                content=content
...
def user_favorites_api(request):
    ...
    try:
        ...
        favorites = TravelPostFavorite.objects.filter(
            user=request.user
        ).select_related('post', 'post__user').prefetch_related('post__cities').order_by('-created_at')
