...
def zip_tool_view(request):
    ...
    ZIP工具主页面
    """
    return render(request, 'tools/zip_tool.html', {
        'title': 'ZIP文件处理工具',
        'description': '支持多文件打包和单文件压缩打包'
    })
...
def compress_single_file_api(request):
    ...
    try:
        ...
        # 压缩文件
        success, message, zip_path = zip_service.compress_single_file(
            file_path=file_path,
            compression_level=compression_level
        )
...
def extract_zip_api(request):
    ...
    try:
        ...
        # 解压文件
        success, message, extract_path = zip_service.extract_zip(
            zip_path=zip_path,
