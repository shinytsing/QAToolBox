#!/bin/bash
# QAToolBox 阿里云服务器一键部署脚本 - 修复版本
# =============================================
# 专为阿里云 47.103.143.152 优化，修复wheel冲突问题
...
install_python() {
    ...
    # 修复pip升级问题，避免wheel冲突
    echo -e "${YELLOW}🔧 修复Python包管理器...${NC}"
    python3 -m pip install --upgrade pip setuptools --break-system-packages --force-reinstall || {
        echo -e "${YELLOW}⚠️  标准升级失败，使用备用方案...${NC}"
        # 备用方案：不升级wheel，只升级pip和setuptools
        python3 -m pip install --upgrade pip setuptools --force-reinstall --ignore-installed
    }
...
setup_python_environment() {
    ...
    # 在虚拟环境中升级pip（避免系统包冲突）
    sudo -u "$PROJECT_USER" .venv/bin/pip install --upgrade pip setuptools
