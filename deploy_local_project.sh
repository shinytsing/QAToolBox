...
# QAToolBox 本地项目直接部署脚本
# 专门用于已有本地项目的情况，跳过Git克隆
...
print_status "🚀 开始QAToolBox本地项目部署"
print_status "💻 模式: 本地项目直接部署（跳过Git克隆）"
print_status "🌐 服务器: 47.103.143.152"
print_status "🔗 域名: shenyiqing.xin"

# 检查当前目录是否为项目目录
CURRENT_DIR=$(pwd)
print_status "📁 当前目录: $CURRENT_DIR"

if [[ ! -f "manage.py" ]]; then
    print_error "当前目录不是Django项目根目录，请切换到包含manage.py的目录"
    exit 1
fi

print_success "✅ 检测到Django项目，开始部署"

# ================================
# [1/9] 系统环境检测和优化
# ================================
print_status "[1/9] 系统环境检测和优化"
...
# [3/9] 配置服务和安全
# ================================
print_status "[3/9] 配置PostgreSQL、Redis、Nginx等服务"
...
# 创建项目目录结构
mkdir -p /home/qatoolbox/{logs,backups}
...
# [5/9] 复制本地项目到部署目录
# ================================
print_status "[5/9] 复制本地项目到部署目录"

# 备份现有项目（如果存在）
if [ -d "/home/qatoolbox/QAToolbox" ]; then
    print_status "🔄 备份现有项目..."
    mv /home/qatoolbox/QAToolbox "/home/qatoolbox/QAToolbox.backup.$(date +%Y%m%d_%H%M%S)"
fi

print_status "📁 复制当前项目到部署目录..."
cp -r "$CURRENT_DIR" /home/qatoolbox/QAToolbox
chown -R qatoolbox:qatoolbox /home/qatoolbox/QAToolbox

print_success "项目复制完成"
...
# [7/9] 安装Python依赖（完整版）
# ================================
print_status "[7/9] 安装Python依赖（完整版，支持所有功能）"

print_status "📋 创建完整依赖列表..."
cat > requirements_local_deploy.txt << EOF
# Django核心框架
Django==4.2.7
...
chown qatoolbox:qatoolbox requirements_local_deploy.txt
...
print_status "📦 安装剩余依赖..."
sudo -u qatoolbox .venv/bin/pip install --timeout 600 -r requirements_local_deploy.txt || {
...
# [8/9] 配置Django生产环境
# ================================
print_status "[8/9] 配置Django生产环境（保持完整功能）"
...
# 生产环境配置（支持完整URL导入）
cat > config/settings/production_local.py << 'EOF'
"""
QAToolBox 生产环境配置（本地项目部署版）
...
chown qatoolbox:qatoolbox config/settings/production_local.py
...
# ================================
# [9/9] 初始化Django应用并启动服务
# ================================
print_status "[9/9] 初始化Django应用并启动服务"

# 设置环境变量
export DJANGO_SETTINGS_MODULE=config.settings.production_local

print_status "🔍 检查Django配置..."
sudo -u qatoolbox DJANGO_SETTINGS_MODULE=config.settings.production_local .venv/bin/python manage.py check --deploy || {
    print_warning "Django检查发现问题，继续执行..."
}

print_status "🗃️ 数据库迁移..."
sudo -u qatoolbox DJANGO_SETTINGS_MODULE=config.settings.production_local .venv/bin/python manage.py makemigrations || print_warning "makemigrations失败，继续..."
sudo -u qatoolbox DJANGO_SETTINGS_MODULE=config.settings.production_local .venv/bin/python manage.py migrate

print_status "👤 创建超级用户..."
sudo -u qatoolbox DJANGO_SETTINGS_MODULE=config.settings.production_local .venv/bin/python manage.py shell << 'EOF'
...
print_status "📁 收集静态文件..."
sudo -u qatoolbox DJANGO_SETTINGS_MODULE=config.settings.production_local .venv/bin/python manage.py collectstatic --noinput
...
environment=DJANGO_SETTINGS_MODULE="config.settings.production_local"
...
# 部署完成报告
# ================================
print_success "🎉 QAToolBox本地项目部署成功！"

cat << EOF

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎊 QAToolBox 本地项目部署成功！今晚可以睡觉了！
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
📋 服务管理命令:
   ...
   • Django管理: sudo -u qatoolbox DJANGO_SETTINGS_MODULE=config.settings.production_local /home/qatoolbox/QAToolbox/.venv/bin/python /home/qatoolbox/QAToolbox/manage.py [command]
...
print_success "本地项目部署脚本执行完成！今晚可以睡个好觉了！🌙"