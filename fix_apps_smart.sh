...
log_info "=========================================="
log_info "QAToolBox 智能修复Django应用配置脚本"
log_info "=========================================="
...
for app in apps/*/; do
    if [ -d "$app" ]; then
        ...
        # 排除__pycache__等非应用目录
        if [[ "$app_name" != "__pycache__" && "$app_name" != "migrations" ]]; then
            if [ -f "$app/__init__.py" ] || [ -f "$app/models.py" ] || [ -f "$app/views.py" ]; then
