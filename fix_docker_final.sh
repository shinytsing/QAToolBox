    ...
    if docker pull registry.cn-hangzhou.aliyuncs.com/library/python:3.12-slim; then
        ...
        if docker pull docker.mirrors.ustc.edu.cn/library/python:3.12-slim; then
            ...
            if docker pull hub-mirror.c.163.com/library/python:3.12-slim; then
                ...
                log_error "所有镜像源都失败，尝试使用本地构建..."
                
                # 方法5: 使用本地构建
                log_info "创建本地Dockerfile..."
                cat > /tmp/Dockerfile.python << 'DOCKERFILE_EOF'
FROM ubuntu:22.04
...
DOCKERFILE_EOF
                ...
                if docker build -t python:3.12-slim /tmp/; then
                    log_success "本地构建成功"
                else
                    log_error "所有方法都失败，请检查网络连接"
                    exit 1
                fi
            fi
        fi
