...
if docker pull python:3.12-slim; then
    ...
    if docker pull registry.cn-hangzhou.aliyuncs.com/library/python:3.12-slim; then
        ...
        # 尝试从中科大拉取
        if docker pull docker.mirrors.ustc.edu.cn/library/python:3.12-slim; then
            log_info "从中科大拉取成功，重新标记镜像..."
            docker tag docker.mirrors.ustc.edu.cn/library/python:3.12-slim python:3.12-slim
            log_success "镜像重新标记成功"
        else
            log_warning "中科大拉取失败，尝试网易镜像..."
            
            # 尝试从网易拉取
            if docker pull hub-mirror.c.163.com/library/python:3.12-slim; then
                log_info "从网易拉取成功，重新标记镜像..."
                docker tag hub-mirror.c.163.com/library/python:3.12-slim python:3.12-slim
                log_success "镜像重新标记成功"
            else
                log_warning "网易拉取失败，尝试百度镜像..."
                
                # 尝试从百度拉取
                if docker pull mirror.baidubce.com/library/python:3.12-slim; then
                    log_info "从百度拉取成功，重新标记镜像..."
                    docker tag mirror.baidubce.com/library/python:3.12-slim python:3.12-slim
                    log_success "镜像重新标记成功"
                else
                    log_warning "所有镜像源都失败，尝试使用现有镜像..."
                    
                    # 检查是否有现有镜像
                    if docker images | grep -q python; then
                        log_info "找到现有Python镜像，使用现有镜像..."
                        # 使用现有的python镜像
                        log_success "使用现有镜像"
                    else
                        log_error "没有找到可用的Python镜像"
                        exit 1
                    fi
                fi
            fi
