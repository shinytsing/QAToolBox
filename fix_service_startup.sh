...
if [ ! -f "QAToolBox/wsgi.py" ]; then
    log_error "QAToolBox/wsgi.py文件不存在"
    exit 1
fi

# 4. 测试gunicorn启动
log_info "测试gunicorn启动..."
timeout 10s /home/admin/QAToolbox/venv/bin/gunicorn --bind 0.0.0.0:8000 --workers 1 --timeout 30 QAToolBox.wsgi:application || {
