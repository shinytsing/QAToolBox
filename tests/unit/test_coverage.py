...
class TestAppsCoverage:
    ...
    def test_model_properties(self):
        ...
        # 测试工具模型属性
        tool_props = [prop for prop in dir(ToolUsageLog) if not prop.startswith('_') and not callable(getattr(ToolUsageLog, prop))]
        assert len(tool_props) > 0
        
        # 测试分享模型属性
        share_props = [prop for prop in dir(ShareRecord) if not prop.startswith('_') and not callable(getattr(ShareRecord, prop))]
